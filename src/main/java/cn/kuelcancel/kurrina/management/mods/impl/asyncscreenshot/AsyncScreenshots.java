package cn.kuelcancel.kurrina.management.mods.impl.asyncscreenshot;
...
import cn.kuelcancel.kurrina.management.mods.impl.AsyncScreenshotMod;
...
public class AsyncScreenshots extends Thread {
        ...
        try {
            ...
            if(AsyncScreenshotMod.getInstance().getMessageSetting().isToggled()) {
            	mc.ingameGUI.getChatGUI().printChatMessage(new ChatComponentText(EnumChatFormatting.UNDERLINE + "Saved screenshot" + EnumChatFormatting.RESET + " ")
            			.appendSibling(new ChatComponentText("[Open] ").setChatStyle(new ChatStyle().setColor(EnumChatFormatting.GOLD).setChatClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, ".kurrina screenshot open " + screenshot.getName())))
            					.appendSibling(new ChatComponentText("[Copy] ").setChatStyle(new ChatStyle().setColor(EnumChatFormatting.BLUE).setChatClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, ".kurrina screenshot copy " + screenshot.getName())))
            							.appendSibling(new ChatComponentText("[Delete]").setChatStyle(new ChatStyle().setColor(EnumChatFormatting.RED).setChatClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, ".kurrina screenshot del " + screenshot.getName())))))));
            }
            
        	if(AsyncScreenshotMod.getInstance().getClipboardSetting().isToggled()) {
        		mc.thePlayer.sendChatMessage(".kurrina screenshot copy " + screenshot.getName());
        	}
        } catch (Exception e) {}
	}
    ...
    public static File getTimestampedPNGFileForDirectory() {
        ...
        while (true) {
            screenshot = new File(
                Kurrina.getInstance().getFileManager().getScreenshotDir(),
