...
import cn.kuelcancel.kurrina.management.mods.impl.mechibes.SoundKey;
...
public class MechvibesMod extends Mod {
	...
	private void loadMouseSounds() {
		Multithreading.runAsync(()->{
			try {
				mouseLeftSound.loadClip(new ResourceLocation("kurrina/mechvibes/mouse.wav"));
				mouseRightSound.loadClip(new ResourceLocation("kurrina/mechvibes/mouse.wav"));
			} catch (Exception e) {}
			loaded = true;
		});
	}
