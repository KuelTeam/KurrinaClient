...
public class ProfileManager {
	...
	public void loadProfiles(boolean loadDefaultProfile) {
		...
		File profileDir = Kurrina.getInstance().getFileManager().getProfileDir();
		int id = 0;
			...
			if(FileUtils.getExtension(f).equals("json")) {
				...
				if(f.getName().equals("Default.json")) {
					...
					try (FileReader reader = new FileReader(f)) {
						...
						KurrinaLogger.error("Failed to load profile", e);
					}
				}
			}
		}
	...
	public void save(File file, String serverIp, ProfileType type, ProfileIcon icon) {
		...
		try(FileWriter writer = new FileWriter(file)) {
			...
			KurrinaLogger.error("Failed to save profile", e);
		}
	}
	
	public void save() {
		save(new File(Kurrina.getInstance().getFileManager().getProfileDir(), "Default.json"), "", ProfileType.ALL, ProfileIcon.GRASS);
	}
