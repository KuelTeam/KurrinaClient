package cn.kuelcancel.kurrina.viaversion.fixes;
    ...
    public static boolean onItemUse(ItemBlock iblock, ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ) {
        ...
        if (stack.stackSize == 0) {
            ...
            if (worldIn.setBlockState(pos, iblockstate1, 3)) {
                ...
                if (ViaVersionMod.getInstance().isToggled() && ViaVersionMod.getInstance().isLoaded() && ViaLoadingBase.getInstance().getTargetVersion().getOriginalVersion() != ViaKurrina.NATIVE_VERSION) {
